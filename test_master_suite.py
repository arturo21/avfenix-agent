...
class TestAVFenixMasterComprehensiveSuite(unittest.TestCase):
    def setUp(self):
        ...
        app_module.app.config['VIDEO_FOLDER'] = os.path.join(self.temp_dir, "video")
        os.makedirs(app_module.app.config['AUDIO_FOLDER'], exist_ok=True)
        os.makedirs(app_module.app.config['VIDEO_FOLDER'], exist_ok=True)
    ...
    def test_10_root_dashboard_route(self):
        res = self.client.get('/')
        self.assertEqual(res.status_code, 200)

    @patch('app.generate_rag_response')
    def test_11_summarize_endpoint(self, mock_rag):
        mock_rag.return_value = ("Resumen ejecutivo completo de prueba.", [], [], "anyapi", "gemini-2.5-flash:free")
        res = self.client.post('/api/summarize', json={"filename": "manual_avfenix.pdf"})
        self.assertEqual(res.status_code, 200)
        data = res.get_json()
    ...
    @patch('app.generate_rag_response')
    @patch('video_generator.WhiteboardVideoGenerator.generate_video')
    def test_12_generate_whiteboard_video(self, mock_vid, mock_rag):
        mock_rag.return_value = ("Resumen para video pizarra.", [], [], "anyapi", "gemini-2.5-flash:free")
        mock_vid.return_value = "whiteboard_test_001.mp4"
        res = self.client.post('/api/generate_whiteboard_video', json={"filename": "manual_avfenix.pdf"})
        self.assertEqual(res.status_code, 200)
        data = res.get_json()
